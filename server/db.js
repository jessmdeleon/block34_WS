...
const createCustomer = async (name) => {
  const id = uuidv4();
  const result = await client.query('INSERT INTO customers (id, name) VALUES ($1, $2) RETURNING *', [id, name]);
  return result.rows[0];
};

const createRestaurant = async (name) => {
  const id = uuidv4();
  const result = await client.query('INSERT INTO restaurants (id, name) VALUES ($1, $2) RETURNING *', [id, name]);
  return result.rows[0];
};
...
const createReservation = async (date, party_count, restaurant_id, customer_id) => {
  const id = uuidv4();
  const result = await client.query('INSERT INTO reservations (id, date, party_count, restaurant_id, customer_id) VALUES ($1, $2, $3, $4, $5) RETURNING *', [id, date, party_count, restaurant_id, customer_id]);
  return result.rows[0];
};
